...
if "时长(秒)" in df.columns:
    ...
    if short_ratio > long_ratio:
        ...
        数据显示，当前样本中短时长历史视频占比较高，3分钟以内视频占比为{short_ratio:.2%}。
        从传播结果看，"{best_duration['时长类型']}"视频的平均播放表现最突出。这说明平台上的历史内容并不只是依赖知识密度获得关注，更依赖节奏压缩、叙事钩子和情绪启动来完成快速传播。
        在这种时长结构下，历史叙事往往呈现出：
        ...
        duration_conclusion = f"""
        数据显示，当前样本中长时长历史视频仍然具有较强存在感，10分钟以上视频占比为{long_ratio:.2%}。

        从传播结果看，"{best_duration['时长类型']}"视频的平均播放表现最突出。这说明历史内容在平台传播中仍保留一定的解释空间，观众并非只接受碎片化内容，也会为具有叙事张力和知识密度的视频投入较长观看时间。
...
if high_emotion_ratio > 0.4:
    ...
    emotion_conclusion = """
    数据显示，大量高传播历史视频呈现出明显的高情绪化特征。这意味着平台传播越来越依赖“高唤醒情绪”推动历史内容扩散。历史叙事正在从传统教材中的“结构解释”模式，逐渐转向以冲突、热血、阴谋与人物对立为核心的“情绪驱动”模式。
...
if top10_ratio > 0.5:
    ...
    up_conclusion = f"""
    数据显示，TOP10历史UP主占据了{top10_ratio:.2%}的总播放量。

    这意味着历史传播存在明显的“头部控制现象”。
